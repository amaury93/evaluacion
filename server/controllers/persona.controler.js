...
const getPersonas = (req, res) => {
    conection_1.default.query('SELECT * FROM persona', (err, data) => {
        if (err)
            throw err;
        res.json(data);
    });
...
const getPersona = (req, res) => {
    const { id } = req.params;
    conection_1.default.query('SELECT * WHERE id = ?', id, (err, data) => {
        if (err)
            throw err;
        res.json(data);
    });
...
const deletePersona = (req, res) => {
    const { id } = req.params;
    conection_1.default.query('DELETE FROM persona WHERE id = ?', id, (err, data) => {
        if (err)
            throw err;
        res.json({
...
const postPersona = (req, res) => {
    const { body } = req;
    conection_1.default.query('INSERT INTO persona set ?', [body], (err, data) => {
        if (err)
            throw err;
        res.json({
...
const putPersona = (req, res) => {
    const { body } = req;
    const { id } = req.params;
    conection_1.default.query('UPDATE persona set ? WHERE id = ?', [body, id], (err, data) => {
        if (err)
            throw err;
        res.json({
